    ...
    def get_segment_size(self, content: bytes) -> int:
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to parse WAV header, using default segment size: {e}")
                # 使用默认值
                return 16000 * 2 * 200 // 1000  # 16kHz, 16-bit, 200ms
        except Exception as e:
...
async def main():
    # 测试文件路径
    file_path = "F:/mycode/project_bot/project_bot/voice/testrecord.wav"
    ...
    async with AsrWsClient(url, segment_duration) as client:  # 使用async with
        try:
            logger.info(f"Starting ASR processing for file: {file_path}")
            logger.info(f"Using WebSocket URL: {url}")
            ...
            async for response in client.execute(file_path):
                ...
                if response.payload_msg and "text" in response.payload_msg:
                    logger.info(f"Recognition result: {response.payload_msg['text']}")
